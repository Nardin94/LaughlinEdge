...
namespace edgeMC {
	...
	////////////////////////////////////////////////////////////////////
	// Compute the metric only: (f|g). Given the rotational-symmetry of the confinement, |f) and |g) have the same angular momentum
	// The code here doesn't exploit the symmetry to set up a block-diagonal matrix: it asks for a single subspace directly
	
		__device__ void montecarlo::metric_sampler( results *dev_results, int target_state ){
			// Get the block id
		...
		__global__ void mcmc_metric(	curandState *state, 
			...
			markovChain.metric_sampler( dev_results, samplingEdgeState );
		...
		cmatrix<double> metric_compute( 	monteCarloParameters &params, // The parameters for the simulation
			...
			float acceptance;
			auto measure = [&] ( int edgeMode, cmatrix<double> &M ){
				...
				mcmc_metric<<<params.nBlocks, params.threadsPerBlock>>>(devState, dev_partitions, edgeMode, params, device_results);
