...
namespace edgeMC {
	...
	////////////////////////////////////////////////////////////////////
	// Compute the spectrum only -- i.e. sample the confinement (and metric) matrix elements:
	// (f|Hg) and (f|g). Given the rotational-symmetry of the confinement, |f) and |g) have the same angular momentum
	// The code here doesn't exploit the symmetry to set up a block-diagonal matrix: it asks for a single subspace directly
	
		__device__ void montecarlo::hamiltonian_sampler( results *dev_results, int target_state ){
			// Get the block id
		...
		__global__ void mcmc_hamiltonian(	curandState *state, 
			...
			markovChain.hamiltonian_sampler( dev_results, samplingEdgeState );
		...
		std::vector<double> spectrum_compute( 	monteCarloParameters &params, // The parameters for the simulation
			...
			float acceptance;
			auto measure = [&] ( int edgeMode, cmatrix<double> &H, cmatrix<double> &M ){
				...
				mcmc_hamiltonian<<<params.nBlocks, params.threadsPerBlock>>>(devState, dev_partitions, edgeMode, params, device_results);
			...
			cmatrix<double> H(params.subspaceDimension, params.subspaceDimension);
			cmatrix<double> M(params.subspaceDimension, params.subspaceDimension);		
			...
			for(int edgeMode=0; edgeMode<P.subspaceDimension; edgeMode++){
				...
				// Measure
				float time = measure( edgeMode, H, M );
			...
			return eig.extract_spectrum();//generalizedEigensolver(H, M);					
		}
